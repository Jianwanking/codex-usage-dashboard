...
- App Group 常量目前写在共享代码里：
  - `group.com.example.codexusagedashboard`
- 共享快照文件名：
  - `quota-snapshot.json`
...
Solution: 安装完整 Xcode 后，用 `xcodebuild -project CodexQuotaDesktop.xcodeproj -scheme CodexQuotaDesktop -configuration Debug CODE_SIGNING_ALLOWED=NO build` 编译，再直接启动构建产物；随后检查 `~/Library/Group Containers/group.com.example.codexusagedashboard/quota-snapshot.json`。
Verification: 宿主 App 已成功启动，进程名为 `CodexQuotaDesktop`；共享快照已实际写入 `~/Library/Group Containers/group.com.example.codexusagedashboard/quota-snapshot.json`，内容包含 `fiveHourRemainingPercent`、`weekRemainingPercent`、`planType` 和 `sourceRolloutPath`。
...
Solution: 给 `Widget/CodexQuotaDesktopWidget.entitlements` 增加 `com.apple.security.app-sandbox = true`，并使用已登录的 Xcode Apple 账号对应 Team `<YOUR_TEAM_ID>` 做正式开发签名；随后安装签名后的 App 到 `/Applications` 并用 `pluginkit -a` 重新注册扩展。
Verification: `pluginkit -m -A -D -p com.apple.widgetkit-extension` 现在能看到 `com.example.CodexUsageDashboard.widget(1.0)`；签名版宿主 App 启动后，`~/Library/Group Containers/group.com.example.codexusagedashboard/quota-snapshot.json` 仍会刷新。
...
Cause: Widget 扩展虽然能解析到 App Group 路径，但实际读取 `~/Library/Group Containers/group.com.example.codexusagedashboard/quota-snapshot.json` 时会报“没有查看权限”；宿主 App 是非 sandbox 的，本地直读 `~/.codex` 没问题，但 Widget 的 sandbox 不能稳定消费这份由宿主写入的 App Group 文件。
Solution: 保留 App Group 作为第一读取路径，同时让宿主 App 额外把同一份快照写进 Widget 自己容器内的 fallback 路径：`~/Library/Containers/com.example.CodexUsageDashboard.widget/Data/Library/Application Support/CodexQuotaDesktop/quota-snapshot.json`。Widget 先尝试 App Group，失败后自动回退到自己容器内的同名快照。
...
Issue: 修复 `limit_id` 后，Widget 仍可能从 `85/84` 回跳到旧的 `92/87`。
Cause: `state_5.sqlite` 的 `threads.updated_at_ms` 只代表线程更新时间，不代表该 rollout 里最新的 `codex` 总额度事件时间。新线程可能复制或保留较旧的 `codex` 额度事件，同时继续写入模型线路额度；如果按 SQLite path 顺序遇到第一个可读快照就返回，会选到较新的线程文件但较旧的总额度。
Solution: 对最近若干 rollout path 全部读取，分别找每个文件的最后一条 `limit_id == "codex"` 总额度事件，再按事件自身 `timestamp` 全局选择最新的一条；只有完全找不到 `codex` 总额度事件时，才按同样方式回退到普通 `token_count`。
Verification: 新增 `testSelectsNewestOverallLimitAcrossRecentRollouts`，复现“第一个 rollout 里 `codex` 是 `92/87`，第二个 rollout 里更新的 `codex` 是 `85/84`”的情况；`swift run CodexQuotaWidgetVerification` 已通过。
Avoid next time: 额度新鲜度要看 `token_count.timestamp`，不要只看 rollout 文件修改时间或 SQLite 线程更新时间。

Issue: 用户继续对话后，Codex UI 已经变化到约 `80/83`，Widget 仍停在 session JSONL 的 `85/84`。
Cause: 新版 Codex 不一定把每次服务端返回的总额度同步写回 rollout `token_count`；当前机器最新 session 里只继续写入 `codex_bengalfox` 模型线路额度，而真正新鲜的总额度在 `~/.codex/sqlite/logs_2.sqlite` 的服务端响应日志里，例如 `x-codex-primary-used-percent` / `x-codex-secondary-used-percent` 和 websocket `codex.rate_limits` 事件。
Solution: 宿主 App 构建快照时优先读取 `logs_2.sqlite` 中最新的服务端总额度日志，解析 HTTP header 或 websocket `codex.rate_limits`；日志不可用或解析不到时，再回退到原来的 `state_5.sqlite -> rollout jsonl -> token_count` 链路。
Verification: 新增 `testBuildFromLocalStatePrefersFreshCodexLogs`，复现“session 仍是旧值但 logs 已有新值”的场景；`swift run CodexQuotaWidgetVerification` 已通过。
Avoid next time: 额度刷新问题先按“Codex UI -> logs_2.sqlite 服务端额度 -> 快照 JSON -> Widget fallback JSON -> 桌面 Widget”顺序查，不要只盯 session JSONL。

Issue: Widget 在 `64/81` 和旧的 `72/82` 之间来回跳。
Cause: `logs_2.sqlite` 里除了真实 `codex.rate_limits` 外，还会记录 assistant 输出、函数调用参数等普通 websocket 文本；这些文本可能包含字符串 `codex.rate_limits`，导致粗筛 SQL 命中假记录。同时 Widget 读取快照时先读到哪个 JSON 就用哪个，旧副本可能压过新副本。
...
Issue: 中号 Widget 在不同机器实际点尺寸略小时，圆环 bitmap 参数可能不随 `ringSize` 缩放。
Cause: 布局层已用 `GeometryReader` 计算响应式 `ringSize`，但 CoreGraphics 绘制仍固定使用 `136pt` 设计稿下的半径和方块尺寸。
Solution: 保留现有布局公式，以 `136pt` 为基准给圆环半径、方块尺寸、圆角和中心文字字号统一乘 `ringSize / 136`。
Verification: `swift run CodexQuotaWidgetVerification` 和 Debug/Release `xcodebuild` 通过后，当前 `136pt` 外观应基本不变，小于 `136pt` 时不裁切。
Avoid next time: Widget 自适应不要只缩容器；自绘 bitmap 的内部半径和块尺寸也必须按同一基准缩放。
